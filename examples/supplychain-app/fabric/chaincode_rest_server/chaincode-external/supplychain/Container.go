...
func (s *SmartContract) createContainer(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	var request common.ContainerRequest
...
func (s *SmartContract) updateContainerCustodian(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if !(container.AccessibleBy(identity)) {
		return peer.Response{
			Status:  403,
			Message: "You are not authorized to perform this transaction since container not accessible by identity",
		}
	}
	...
	if container.ContainerID != "" {
		...
		var outercontainer common.Container
		...
		if outercontainer.Custodian != newCustodian {
			return peer.Response{
				Status:  403,
				Message: "Container needs to be unpackaged before claiming a new owner",
			}
		}
	}
	...
	var recup func(common.Container) peer.Response
	recup = func(container common.Container) peer.Response {
...
func (s *SmartContract) packageItem(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if containerID == contentID {
		return peer.Response{
			Status:  404,
			Message: "Cannot package item into itself, please choose another container",
		}
	}
	...
	if err != nil && err.Error() == "Not a Product" {
		...
		if !(contentContainer.ContainerID == "") {
			return peer.Response{
				Status:  403,
				Message: "You are not authorized to perform this transaction as containerID is not empty for container",
			}
		}
		...
		if !(identity.Cert.Subject.String() == contentContainer.Custodian) {
			return peer.Response{
				Status:  403,
				Message: "You are not authorized to perform this transaction as cert.subject.string doesn't equal custodian for container ",
			}
		}
		//set new data
		...
		if !(identity.Cert.Subject.String() == contentProduct.Custodian) {
			return peer.Response{
				Status:  403,
				Message: "You are not authorized to perform this transaction as cert.subject.string doesn't equal custodian for product while packaging",
			}
		}
		if !(contentProduct.ContainerID == "") {
			return peer.Response{
				Status:  403,
				Message: "You are not authorized to perform this transaction as containerID is not empty for product",
			}
		}
		//set new data
	...
	if !(identity.Cert.Subject.String() == container.Custodian) {
		return peer.Response{
			Status:  403,
			Message: "You are not authorized to perform this transaction as string doesn't match custodian for container while packaging",
		}
	}
...
func (s *SmartContract) unpackageItem(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if !(identity.Cert.Subject.String() == contentProduct.Custodian) {
		return peer.Response{
			Status:  403,
			Message: "You are not authorized to perform this transaction as cert.subject.string doesn't equal custodian for product while unpackaging",
		}
	}
	if !(contentProduct.ContainerID == containerID) {
		return peer.Response{
			Status:  403,
			Message: "Product not located in this container, could not be unpackaged",
		}
	}
	//set new data
	...
	if err != nil {
		...
		if !(contentContainer.ContainerID == containerID) {
			return peer.Response{
				Status:  403,
				Message: "Container not located in this container, could not be unpackaged",
			}
		}
		...
		if !(identity.Cert.Subject.String() == contentContainer.Custodian) {
			return peer.Response{
				Status:  403,
				Message: "You are not authorized to perform this transaction as cert.subject.string doesn't equal custodian for contentContainer while unpackaging",
			}
		}
		//set new data
	...
	if !(identity.Cert.Subject.String() == container.Custodian) {
		return peer.Response{
			Status:  403,
			Message: "You are not authorized to perform this transaction as cert.subject.string doesn't equal custodian for container while unpackaging",
		}
	}
